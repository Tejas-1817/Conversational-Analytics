    ...
    for table in tables:
        ...
        try:
            with engine.connect() as conn:
                row_count = conn.execute(text(f"SELECT count(*) FROM {fq_table}")).scalar_one()
                table.row_count = int(row_count)

                for column in (c for c in table.columns if c.is_active):
                    col = _quote(engine, column.column_name)
                    sample = f"(SELECT {col} AS v FROM {fq_table} LIMIT {settings.profile_sample_rows}) s"

                    base = conn.execute(text(
                        f"SELECT count(*), count(v), count(DISTINCT v) FROM {sample}"
                    )).one()
                    ...
                    if any(h in column.data_type.lower() for h in _ORDERABLE_HINTS):
                        mn, mx = conn.execute(text(f"SELECT min(v), max(v) FROM {sample}")).one()
                        profile["min"], profile["max"] = str(mn), str(mx)

                    top = conn.execute(text(
                        f"SELECT v, count(*) AS c FROM {sample} WHERE v IS NOT NULL "
