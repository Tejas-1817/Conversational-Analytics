...
curl -s -X POST localhost:8000/sources -H "X-API-Key: change-me" -H "Content-Type: application/json" -d '{
...
curl -s -X POST localhost:8000/jobs/ingest/<source_id> -H "X-API-Key: change-me"

# 3. Watch the job, then browse results
curl -s localhost:8000/jobs/<job_id> -H "X-API-Key: change-me"
curl -s localhost:8000/metadata/sources/<source_id>/tables -H "X-API-Key: change-me"
curl -s localhost:8000/metadata/sources/<source_id>/relationships -H "X-API-Key: change-me"
```
