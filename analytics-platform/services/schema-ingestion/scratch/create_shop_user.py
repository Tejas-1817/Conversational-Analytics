...
e = create_engine("postgresql+psycopg://demo:demo@host.docker.internal:5443/shop").execution_options(isolation_level="AUTOCOMMIT")
with e.connect() as conn:
    try:
        conn.execute(text("CREATE USER readonly_user WITH PASSWORD 'readonly';"))
    except Exception as e:
        print(f"User might exist: {e}")
    conn.execute(text("GRANT CONNECT ON DATABASE shop TO readonly_user;"))
...
print("Read-only user created/granted on demo DB!")
